...
function istaco(Player)
    return Player.PlayerData.job.name == 'taco'
end
...
if ox_inv then
    for i = 1, #Config.Zones do
        ...
        elseif v.type == 'shop' then
            exports.ox_inventory:RegisterShop('Ingredients - Taco', {
                name = 'Ingredients - Taco',
                inventory = {
    ...
    for k,v in pairs(Config.HungerFill) do
        exports(k, function(event, item, inventory, slot, data)
            if event == 'usingItem' then
                TriggerClientEvent('lotus_taco:client:Eat', inventory.id, k, slot, v.emote)
            end
        end)
    end
    for k,v in pairs(Config.ThirstFill) do
        exports(k, function(event, item, inventory, slot, data)
            if event == 'usingItem' then
                TriggerClientEvent('lotus_taco:client:Drink', inventory.id, k, slot, v.emote)
            end
        end)
    end
else
    for k,v in pairs(Config.HungerFill) do
        QBCore.Functions.CreateUseableItem(k, function(source, item)
            TriggerClientEvent('lotus_taco:client:Eat', source, k, item.slot, v.emote)
        end)
    end
    for k,v in pairs(Config.ThirstFill) do
        QBCore.Functions.CreateUseableItem(k, function(source, item)
            TriggerClientEvent('lotus_taco:client:Drink', source, k, item.slot, v.emote)
        end)
    end
end